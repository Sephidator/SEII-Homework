...
|clietbl模块提供的服务（供接口）|||
|---|---|---|
|clientblservice.findClient|语法|public ArraytList<Client> findClient(String ID)|
||前置条件|	id符合输入条件|
||后置条件|查找到目标客户|
|clientblservice.addClient|语法|xx|
||前置条件|xx|
||后置条件|xx|
<br/>
...
|---|---|
|服务名|服务|
|clientDataService.findClient(int id)|根据ID进行查找单一持久化对象|
<br/>
