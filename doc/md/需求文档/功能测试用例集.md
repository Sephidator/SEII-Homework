...
- [功能测试用例集](#功能测试用例集)
    ...
    - [用例13 修改进货类单据](#用例13-修改进货类单据)
        ...
        - [表13-3 TUS1的测试用例](#表13-3-tus1的测试用例)
    ...
    - [用例25 促销策略管理](#用例25-促销策略管理)
        ...
        - [表25-3 TUS1的测试用例](#表25-3-tus1的测试用例)
...
| 编号   | 测试用例套件1 | 测试用例套件2 | 测试用例套件3 |
| ---------------------------------------- | ---- | ---- | ---- |
| RecAndPayBill.Input                      | TUS1 | TUS2 |      |
| RecAndPayBill.Input.Empty                | TUS1 | TUS2 |      |
| RecAndPayBill.Input.Cancel               | TUS1 |      |      |
| RecAndPayBill.Input.Type                 | TUS1 | TUS2 | TUS3 |
| RecAndPayBill.Input.Type.Sign            | TUS1 | TUS2 | TUS3 |
| RecAndPayBill.Input.Serial.Full          | TUS1 | TUS2 |      |
| RecAndPayBill.Input.Client               | TUS1 | TUS2 |      |
| RecAndPayBill.Input.Client.Empty         | TUS1 | TUS2 |      |
| RecAndPayBill.Input.TransList            | TUS1 | TUS2 |      |
| RecAndPayBill.Client.Empty               | TUS1 |      |      |
| RecAndPayBill.TransList.Empty            | TUS1 |      |      |
| RecAndPayBill.TransList.Robot            | TUS1 |      |      |
| RecAndPayBill.TransList.BankAccount      | TUS1 |      |      |
| RecAndPayBill.TransList.BankAccount.Add  | TUS1 |      |      |
| RecAndPayBill.TransList.BankAccount.Empty| TUS1 |      |      |
| RecAndPayBill.TransList.Money.Empty      | TUS1 |      |      |
| RecAndPayBill.TransList.Money.Invalid    | TUS1 |      |      |
| RecAndPayBill.Robot.AddOperator          | TUS1 |      |      |
| RecAndPayBill.Robot.Calc                 | TUS1 |      |      |
| RecAndPayBill.Store                      |      | TUS2 | TUS3 |
| RecAndPayBill.Store.Draft                |      | TUS2 |      |
| RecAndPayBill.Store.Submit               |      | TUS2 |      |
| RecAndPayBill.Status                     |      | TUS2 | TUS3 |
| RecAndPayBill.Status.Draft               |      | TUS2 |      |
| RecAndPayBill.Status.Examining           |      | TUS2 |      |
| RecAndPayBill.Status.Pass                |      |      | TUS3 |
| RecAndPayBill.Status.Reject              |      |      | TUS3 |
| RecAndPayBill.End                        |      | TUS2 |      |
| RecAndPayBill.End.Update                 |      | TUS2 |      |
| RecAndPayBill.End.Close                  |      | TUS2 |      |
| RecAndPayBill.Update                     |      | TUS2 | TUS3 |
| RecAndPayBill.Update.Self                |      | TUS2 | TUS3 |
| RecAndPayBill.Update.Log                 |      | TUS2 | TUS3 ||
...
| ID     | 输入   | 输入   | 输入                | 输入                                       | 输入   |预计输出                                     |
| ------ | ---- | ---- | ----------------- | ------- | ---- | ------- |
|        | 单据类型 | 单据上限 | 客户  | 转账列表 | 是否取消 | |
| TUS1-1 | 收款单  | 是    | 无| 无 | 否    |系统提示单据数量已达上限 |
| TUS1-2 | 收款单  | 否    | 无| 银行账户：6221222402017123012<br/>转账金额：10000元<br/>备注：无 | 否    |系统提示没有选择客户 、 |
| TUS1-3 | 付款单  | 否    | 供应商：王五 | 银行账户：无<br/>转账金额：0元<br/>备注：无 | 是    |系统提示转账列表为空 |
| TUS1-4 | 付款单  | 否    | 供应商：王五 | 银行账户：6221222402017123012<br/>转账金额：10123元（注：该账户实际余额只有10000元）<br/>备注：无 | 否    |系统提示输入的转账金额不得大于账户余额 |
| TUS1-5 | 付款单  | 否    | 供应商：王五 | 银行账户：6221222402017123012<br/>转账金额：10000元）<br/>备注：“一次偿清”   | 否    |系统添加当前操作员信息（登陆者），并输出总额汇总10000元 |
| TUS1-6 | 收款单  | 否    | 销售商：李四 | 银行账户：6221222402017123099<br/>转账金额：10000元<br/>备注：无 | 是    | 系统抛弃单据信息并关闭制定收付款单事务 |

### 表15-4 TUS2测试用例
| ID     | 输入   | 输入   | 输入   | 输入   | 预计输出                        |
| ------ | ---- | ---- | ---- | ---- | --------------------------- |
|        | 单据类型 | 填写情况 | 保存方式 | 是否结束 |                             |
| TUS2-1 | 收款单  | 完整且正确    | 保存草稿    | 是    | 系统将单据标记为草稿状态，持久化保存该单据，并退出制定收付款单事务|
| TUS2-2 | 付款单  | 完整且正确    | 提交单据    | 是    | 系统将单据标记为待审批状态，持久化保存该单据，记录操作日志，并退出制定收付款单事务 |
| TUS2-3 | 收款单  | 完整且正确    | 提交单据    | 否    | 系统将单据标记为待审批状态，持久化保存该单据，记录操作日志 |

### 表15-5 TUS3测试用例
| ID     | 输入   | 输入     |     预计输出                |
| ------ | -----  |---------| -------------------------- |
|        |单据类型       |     单据审批情况     |     |
| TUS3-1 | 收款单 |审批通过      | 系统将单据标记为审批通过状态，向财务人员发送单据消息，持久化更新单据信息 |
| TUS3-2 | 付款单 |审批不通过    | 系统将单据标记为审批不通过状态，持久化更新单据信息 |
