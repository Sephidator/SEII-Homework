...
| Goods.Add.Finish          | 库存管理人员输入添加的商品属性（输入），包括编号、名称、型号、数量、备注、所在分类、警戒数量、进价、零售价、最近进价、最近零售价、，选择完成（查询），系统更新商品信息（逻辑文件） |
...
## 4.6 建立库存赠送单
	...
	逻辑文件：6
	对外接口：0
	功能点测度总数：4*4+8*5+5*4+6*10+0*7=136
	FP=136*（0.65+0.01*40）=142.8

| 编号                                  | 需求描述与度量                                  |
| ----------------------------------- | ---------------------------------------- |
| InventoryGiftBill.Input              | 系统应该允许库存管理人员在进行建立库存赠送单时（输出：建立库存赠送单主界面）既可以鼠标点击，又可以键盘输入 |
| InventoryGiftBill.Input.Add          | 库存管理人员输入建立库存赠送单命令时（查询），系统允许库存管理人员修改商品赠送数量，参见InventoryGiftBill.Add |
| InventoryGiftBill.Input.Show         | 库存管理人员输入建立库存赠送单命令时（查询），系统允许显示商品列表，参见InventoryGiftBill.Show |
| InventoryGiftBill.Input.Update       | 库存管理人员输入建立库存赠送单命令时（查询），系统允许库存管理人员更新日志与单据，参见InventoryGiftBill.Update |
| InventoryGiftBill.Show               | 系统允许库存管理人员建立库存赠送单，系统将会显示商品的列表（按照保存的先后顺序）（输出）（逻辑文件） |
| InventoryGiftBill.Show.Search        | 当库存管理人员欲进行特定商品的查询时，可以选择输入：名称、商品编号、商品分类（输入），进行查询（查询），之后系统显示列表列出符合条件的商品（输出）（逻辑文件） |
| InventoryGiftBill.Show.Search.Warning | 库存管理人员进行商品查询时未输入条件就进行查询，系统提示未输入条件（输出）    |
| InventoryGiftBill.Show.Return        | 库存管理人员选择返回（查询），开始新任务                     |
| InventoryGiftBill.Add                | 系统允许库存管理人员在建立库存赠送单界面添加商品的赠送数量              |
| InventoryGiftBill.Add.GetData        | 库存管理人员选择商品时，系统自动获取并显示商品数量（逻辑文件）（输出）      |
| InventoryGiftBill.Add.Warning.Negative | 库存管理人员赠送数量输入负数（输入），系统提示数量不能为负数（输出）       |
| InventoryGiftBill.Add.Warning.Decimal | 库存管理人员赠送数量输入小数（输入），系统提示数量不能为小数（输出）       |
| InventoryGiftBill.Add.Warning.Over | 库存管理人员输入赠送数量大于商品信息（输入），系统提示赠送数量不能大于商品数量（输出）       |
| InventoryGiftBill.Add.Return         | 库存管理人员选择返回（查询），开始新任务                     |
