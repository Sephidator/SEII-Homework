...
## 4.15 制定收付款单
	...
	逻辑文件：15
	对外接口：3
	功能点测度总数 = 15*4 + 13*5 + 0*4 + 15*10 + 3*7 = 296
	FP = 296*(0.65 + 0.01*42) = 316.72

	| 编号                                       | 需求描述与度量                                  |
	| ---------------------------------------- | ---------------------------------------- |
	| RecAndPayBill.Input                      | 系统应该允许财务人员在制定收付款单时进行键盘输入（输入）             |
	| RecAndPayBill.Input.Empty              | 当输入的数据不完整时（输入），系统不允许保存提交                 |
	| RecAndPayBill.Input.Cancel               | 财务人员取消新建单据时，系统抛弃输入的数据，关闭制定收付款单任务，返回财务单据任务   |
	| RecAndPayBill.Input.Type                 | 财务人员根据需求选择制定的单据类型（输入），包括收款单和付款单              |
	| RecAndPayBill.Input.Type.Sign            | 财务人员选择制定的单据类型后，系统自动填充单据类型为“收款单”或者“付款单”（输出） |
	| RecAndPayBill.Input.Serial.Full          | 当单据编号后5位已达上限，系统提示单据已达上限（输出），并关闭制定收付款单任务，返回财务管理任务 |
	| RecAndPayBill.Input.Client               | 财务人员选择输入客户信息时（输入），系统检查输入的客户信息              |
	| RecAndPayBill.Input.Client.Empty          | 财务人员没有输入的客户信息时（输入），系统提示客户信息不完整（输出）       |
	| RecAndPayBill.Input.TransList            | 财务人员输入转账列表信息时（输入），系统检查所填写的信息，参见RecAndPayBill.TransList |
	| RecAndPayBill.Client.Empty             | 财务人员未选择输入客户（包括供应商和销售商）（输入）时，系统提示未选择客户（输出） |
	| RecAndPayBill.TransList.Empty             | 财务人员输入转账列表信息时（输入），如果没有输入完整除备注以外的其它信息，系统提示输入不完整（输出） |
	| RecAndPayBill.TransList.Robot            | 财务人员制定收付款单任务并填写信息时，系统自动附加操作员信息（输出），并自动由转账列表计算并添加总额汇总（输出），参见RecAndPayBill.Robot |
	| RecAndPayBill.TransList.BankAccount      | 财务人员输入转账列表中的银行账户时（输入），系统提供当前银行账户列表       |
	| RecAndPayBill.TransList.BankAccount.Add      | 财务人员在账户列表中选择银行账户时（输入）如果没有选择账户就确认添加，系统提示未选择账户（输出）       |
	| RecAndPayBill.TransList.BankAccount.Empty | 财务人员在输入转账列表的银行账户留空时（输入），系统提示银行账户不能为空（输出） |
	| RecAndPayBill.TransList.Money.Empty       | 财务人员在输入转账列表的转账金额留空时（输入），系统提示转账金额不能为空（输出） |
	...
	| RecAndPayBill.Store                      | 财务人员保存或者提交单据时，系统要持久化保存新建的收、付款单（逻辑文件）         |
	| RecAndPayBill.Store.Draft               | 财务人员保存单据时，系统持久化保存草稿状态的该单据（逻辑文件），参见RecAndPayBill.Status |
	| RecAndPayBill.Store.Submit              | 财务人员提交单据时，系统持久化保存待审批状态的该单据（逻辑文件），参见RecAndPayBill.Status |
	...
	| RecAndPayBill.Status.Pass                | 系统将单据标记为“审批通过”时，需要向财务人员发送单据信息（对外接口），更改客户应收或者应付（对外接口），参见客户管理的需求规格，然后持久化保存单据信息（逻辑文件） |
	...
	| RecAndPayBill.Update                     | 系统更新重要数据（逻辑文件），在财务人员保存或者提交单据时，系统才会更新单据信息，仅当提交单据结束时更新操作日志（逻辑文件） |
	| RecAndPayBill.Update.Self                | 系统更新持久化保存的单据数据（逻辑文件）                           |
